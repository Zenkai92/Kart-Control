...
/*    public function getAllCompo()
    {
        try {
            $req = $this->db->query("SELECT * FROM `compo`");
            $data = $req->fetchAll();
            foreach ($data as $datas) {
                $compo = new compo($datas);
                echo "{$compo->getId()} {$compo->getBodies()} {$compo->getTires()} {$compo->getGliders()} {$compo->getDrivers()} {$compo->getUserId()}<br>";
            }
